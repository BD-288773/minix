...
/* A process has blocked. */
  ...
  } else if (isuserp_a(rp)){
	if ( (xp = rdy_head[USER_AQ]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[USER_AQ] = xp->p_nextready;
		if (rp == proc_ptr)
		pick_proc();
		return;
	}
	qtail = &rdy_tail[USER_AQ];
  } else {
	if ( (xp = rdy_head[USER_BQ]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[USER_BQ] = xp->p_nextready;
		if (rp == proc_ptr)
		pick_proc();
		return;
	}
	qtail = &rdy_tail[USER_BQ];
  }
 ...
 */
  ...
  rdy_tail[SERVER_Q + proc_ptr->uq_group]->p_nextready = rdy_head[SERVER_Q + proc_ptr->uq_group];
  rdy_tail[SERVER_Q + proc_ptr->uq_group] = rdy_head[SERVER_Q + proc_ptr->uq_group];
